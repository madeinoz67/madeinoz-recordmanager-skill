...
- ALL tag assignments MUST come from TaxonomyExpert.getTagCategories(domain)
- ALL document types MUST come from TaxonomyExpert.getDocumentTypes(domain)
- DO NOT assign tags that are not in TaxonomyExpert - this breaks compliance
...
3. Get valid tags from TaxonomyExpert.getTagCategories(domain)
4. Determine optimal tag assignments from TaxonomyExpert ONLY
5. Configure storage path routing
6. Set document type auto-classification from TaxonomyExpert.getDocumentTypes(domain)
7. Calculate expected match rate
8. Generate workflow configuration
...
CRITICAL TAXONOMY ENFORCEMENT:
- ALL tag identifications MUST validate against TaxonomyExpert.getTagCategories(domain)
- DO NOT identify patterns for tags that are not in TaxonomyExpert
- Use TaxonomyExpert as single source of truth
...
Validate all tags against TaxonomyExpert.getTagCategories(domain).
