...
- ALL document type suggestions MUST come from TaxonomyExpert.getDocumentTypes(domain)
- DO NOT invent or hardcode tags - retrieve from taxonomy only
- Validate all existing tags against TaxonomyExpert - flag any NOT in taxonomy for removal
...
3. Get valid document types from TaxonomyExpert.getDocumentTypes(domain)
4. Identify taxonomy gaps and inconsistencies
5. Suggest tag hierarchy improvements ONLY from TaxonomyExpert
6. Recommend document type corrections ONLY from TaxonomyExpert
7. Flag tags NOT in TaxonomyExpert for removal
8. Propose naming convention standards
9. Calculate organization score (0-100)
...
CRITICAL TAXONOMY ENFORCEMENT:
- ALL tag standardization MUST use tags from TaxonomyExpert.getTagCategories(domain)
- DO NOT create custom tags - ONLY use what exists in taxonomies.yaml
- Storage paths should follow domain conventions from TaxonomyExpert
- Use TaxonomyExpert as single source of truth

Your task:
1. Design comprehensive organization architecture
2. Plan cross-domain tag standardization using TaxonomyExpert.getTagCategories()
3. Create storage path strategy based on TaxonomyExpert domain structure
