...
- Validate all current tags exist in TaxonomyExpert - flag invalid tags for removal
- Use TaxonomyExpert as single source of truth

HIERARCHICAL FSA TAG REQUIREMENT (MANDATORY):
- ALL hierarchical taxonomy tags MUST use FULL FSA path format
- Format: "Function/Service/Activity" (e.g., "PetCare/EndOfLifeCare/Cremation")
- NEVER use individual level tags (e.g., DO NOT use "PetCare", "EndOfLifeCare", "Cremation" separately)
- Use TaxonomyExpert.generateHierarchicalTags() to get correct full path format
- Example: "HealthManagement/MedicalCare/DoctorVisits" NOT "HealthManagement" + "MedicalCare" + "DoctorVisits"
- Each document MUST have exactly ONE full FSA path tag for primary classification

FSA → RETENTION COMPLIANCE (CRITICAL):
- When adding/changing FSA tags, MUST automatically apply retention tags
- Retention period determined from FSA Activity level via TaxonomyExpert.getRetentionForActivity()
- FSA path and retention tag MUST be applied in single atomic operation
- NEVER apply FSA tags without corresponding retention tags
- Example: FSA/FinancialRecords/Banking/AccountStatements → RETENTION/7-years
...
2. Get valid tags from TaxonomyExpert.getTagCategories(domain)
3. Suggest tags ONLY from TaxonomyExpert - no custom tags
4. If suggesting FSA tags, MUST also look up and include retention tag via TaxonomyExpert.getRetentionForActivity()
5. Flag any current tags NOT in TaxonomyExpert for removal
6. Identify mandatory tags for document type from taxonomy
7. Explain reasoning for each suggestion with taxonomy category reference
...
**After receiving tag suggestions, validate against TaxonomyExpert:**

```typescript
import { TaxonomyValidator } from '../Lib/TaxonomyValidator.js';

// Validate all suggested tags
const allSuggestedTags = [
  ...recordsKeeperResponse.mandatoryTags,
  ...recordsKeeperResponse.suggestedTags,
  ...recordsKeeperResponse.optionalTags
];

const validation = TaxonomyValidator.validateAgentSuggestions(
  { tags: allSuggestedTags },
  country,
  domain
);

if (!validation.valid) {
  console.error('❌ Tag suggestions failed validation:');
  ...
  throw new Error('Tag suggestions not in TaxonomyExpert - aborting tagging operation');
}

console.log('✅ All tag suggestions validated against TaxonomyExpert');
```
