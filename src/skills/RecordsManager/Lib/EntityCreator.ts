...
export class EntityCreator {
  ...
  constructor(paperlessClient: PaperlessClient, taxonomyExpert: TaxonomyExpert) {
    ...
    // Use PAI_DIR for entity registry, fallback to .claude
    const paiDir = process.env.PAI_DIR || `${process.env.HOME}/.claude`;
    this.registryPath = `${paiDir}/skills/RecordsManager/Context/entities.json`;
  }
  ...
  private async createStoragePath(config: EntityConfig): Promise<StoragePath | null> {
    // Sanitize entity name for use in storage paths
    // Replace forward slashes with hyphens to avoid path parsing issues
    // (e.g., "Darklight Productions P/L" becomes "Darklight Productions P-L")
    const sanitizedName = config.name.replace(/\//g, '-');

    const pathMap: Record<EntityType, (c: EntityConfig) => string | null> = {
      household: (c) => `/Household/${sanitizedName}`,
      corporate: (c) => `/Corporate/${sanitizedName}`,
      'unit-trust': (c) => `/Trusts/Unit Trusts/${sanitizedName}`,
      'discretionary-trust': (c) => `/Trusts/Discretionary Trusts/${sanitizedName}`,
      'family-trust': (c) => `/Trusts/Family Trusts/${sanitizedName}`,
      project: (c) => `/Projects/${sanitizedName}`,
      person: (c) => `/Household/People/${sanitizedName}`,
    };
    ...
    if (!path) return null; // Strict null check for type safety
  ...
  private async createCustomFields(config: EntityConfig): Promise<number[]> {
    ...
    let fieldsToCreate: Array<{ name: string; data_type: 'string' | 'number' | 'date' }> = [];

    // Person-specific fields
    if (config.type === 'person' && config.personConfig) {
      fieldsToCreate = [
        { name: `Full Name (${config.name})`, data_type: 'string' as const },
        { name: `Relationship (${config.name})`, data_type: 'string' as const },
        { name: `Date of Birth (${config.name})`, data_type: 'date' as const },
        { name: `Email (${config.name})`, data_type: 'string' as const },
        { name: `Phone (${config.name})`, data_type: 'string' as const },
      ];
    }
    // Trust-specific fields
      ...
      // Common trust fields
      const commonFields = [
        { name: `ABN (${config.name})`, data_type: 'string' as const },
        { name: `TFN (${config.name})`, data_type: 'string' as const },
        { name: `Trustee Name (${config.name})`, data_type: 'string' as const },
        { name: `Trust Deed Date (${config.name})`, data_type: 'date' as const },
      ];

      // Type-specific fields
      let typeFields: Array<{ name: string; data_type: 'string' | 'number' | 'date' }> = [];
      ...
      if (config.type === 'family-trust') {
        ...
        typeFields = [
          { name: `Beneficiaries (${config.name})`, data_type: 'string' as const },
        ];
      }
