...
Your task:
1. Calculate FTE retention end date (5 years from FTE date)
2. Calculate days remaining until retention ends
3. Determine FTE document status:
   - ACTIVE: Within retention period, must retain
   - APPROACHING: Within 90 days of retention end
   - EXPIRED: Past 5-year retention period
4. Check if FTE document exists in paperless-ngx
5. Verify FTE date custom field is set on entity
6. Generate retention compliance status

IMPORTANT: FTE retention is 5 years from FTE DATE (not EOFY).
...
Your task:
1. Verify FTE complies with Section 272-80 ITAA 1936
2. Check if FTE was made within required timeframe
3. Verify FTE is in writing and signed
4. Check for FTE revocation requirements
5. Assess impact if FTE document is lost/deleted
6. Provide ATO compliance assessment
